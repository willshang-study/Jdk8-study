...
public class WordsCounterTest {
    public static void main(String[] args) {
        ...
        // 获取words的Stream<Character>
        Stream<Character> stream = words.chars().mapToObj(item->new Character((char)item));

        Stream<Character> streamParallel = StreamSupport.stream(new WordCounterSpliterator(words,0,words.length()),true);

        // 需要对这个流做规约操作，即 reduce
        WordsCounter sum = streamParallel.reduce(new WordsCounter(0, true),
    ...
    static class WordsCounter{
        ...
        final int counter;
        // 2.boolean lastSpace：用来记得上一个Character是不是空格
        final boolean lastSpace;

        //  U identity
        WordsCounter(int counter, boolean lastSpace) {
        ...
        public static WordsCounter accumulator(WordsCounter wordsCounter, Character item){
            ...
            if(Character.isWhitespace(item)){
                ...
                return new WordsCounter((wordsCounter.lastSpace? wordsCounter.counter+1:wordsCounter.counter),false);
            }
        }
