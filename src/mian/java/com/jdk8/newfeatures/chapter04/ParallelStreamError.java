...
public class ParallelStreamError {
    ...
    public static void main(String[] args) {
        ...
        OptionalLong timeIterateError = LongStream.range(0,3).map(item->{
            long startTime = System.currentTimeMillis();
            IntStream.iterate(0,a->++a).limit(10000000).parallel().reduce(Integer::sum);
            long period = System.currentTimeMillis() - startTime;
            return period;
        }).min();

        OptionalLong timeRange = LongStream.range(0,3).map(item->{
            long startTime = System.currentTimeMillis();
